...
```
Now, the existence of the time split identified in the Exploratory data analysis section informs us to posit the model, with the same notation,
...
We will now compare models by their estimated mean squared prediction error. We will estimate this quantity with a leave-one-out cross-validation procedure that computes mean squared errors because it is flexible enough to adapt to the other model-fitting approaches that will be used in this study. The CV scores of the previous two full models are as follows. It is clear that the model that recognizes the time split performs much better.
...
cv_fullmod
print("CV standard error of full model, no time split")
sd(errors1)/sqrt(nrow(covid))

cv_fullmod1 <- mean(errors2)
print("CV score of full model with time split")
cv_fullmod1
print("CV standard deviation of full model with time split")
sd(errors2)/sqrt(nrow(covid))
```
...
print("CV score and standard deviation of LASSO model, no time split")
lasso_mod$cvm[lasso_mod$lambda == lasso_mod$lambda.min]
lasso_mod$cvsd[lasso_mod$lambda == lasso_mod$lambda.min]
```
...
print("CV score and standard deviation of LASSO model with time split")
...
print("CV score and standard deviation of ridge model, no time split")
...
print("CV score and standard deviation of ridge model with time split")
...
To judge if non-linear transformations of data are necssary, we will consult the residual and QQ plots of the stepwise-selected model.
