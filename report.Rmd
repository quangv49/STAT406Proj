...
```
The stepwise selection procedure ended up selecting a model which omits `resto_visit` and `public_transit` as well as interaction terms of the time split with `bar_visit`, and `large_events` while keeping the time split. To correctly assess the quality of this model, we must perform a more elaborate CV procedure.
