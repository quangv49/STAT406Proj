...
We first examine the descriptive statistics of the response and all predictors. This is done by looking at their boxplots, as in Figure \@ref(fig:indiv_boxplots).

```{r indiv_boxplots, echo=FALSE, fig.height=8, fig.cap = "Boxplots of individual predictors and COVID-19 case counts."}
...
We plot all predictors against the number of cases in Figure \@ref(fig:plots_against_cases).

```{r plots_against_cases, echo=FALSE, fig.height = 8, fig.cap = "Scatterplots of predictors against case counts."}
...
```{r eda-tree2, echo = F, fig.height=3, fig.cap = "Regression tree to determine clusters in data not determined by time."}
eda_tree2 <- rpart(cases ~ .-date-time, data = covid, method = "anova")
plot(eda_tree2, uniform = F, margin = 0.5)
text(eda_tree2, pretty = T)
```

With the two splitting criteria, we can plot the predictors against cases again. The first set of plots in Figure \@ref(fig:plot-time-split) is when the dataset is split by date. The second set of plots in Figure \@ref(fig:plot-mask-split) is when the dataset is split by `mask_prop`.
...
```{r plot-mask-split, echo = F, fig.height=8, fig.cap = "Scatterplots of predictors against case counts for dates such that `mask_prop` < 70.67 ."}
par(mfrow=c(3,3))
